...
const INPUT_ROOMOBJECT = document.getElementById("input-roomObject");
const INPUT_ROOMOBJECT_ENABLED = document.getElementById("input-roomObject-enabled");
...
for (var y = 0; y < yMax; y++) { //create grid of wrappers/spans, xMax by yMax in dimension
    ...
    var tileBreak = document.createElement("br");
    tileBreak.style.clear = "both"; //create linebreak for div rows
...
function pasteTiles() {
    ...
    for (var i = 0; i < copiedTileArray.length; i++) {
        try {
            ...
            rooms[id].roomObject = copiedTileArray[i].roomObject;
...
function paintTile(id) {
    ...
    if (INPUT_ROOMOBJECT_ENABLED.checked === true) { //if namerush is enabled, change the name of the tile
        tile.roomObject = INPUT_ROOMOBJECT.value;
        console.log("painted");
    }
}
...
function loadMap() {
    ...
    try {
        for (var i = 0; i < nodes.length; i++) { //iterate over list of room nodes and load room properties
            ...
            rooms[i].roomObject = nodes[i].getElementsByTagName("object")[0].innerHTML; //load roomobject
...
function saveMap() {
    ...
    for (var i = 0; i < rooms.length; i++) {
        ...
        xw.writeElementString('object', rooms[i].roomObject);
